...
const altNumbers = (numArray) => {
    ...
    const result = [];
    const lPos = positives.length;
    const lNeg = negatives.length;
    let getPositive = lPos >= lNeg;
    let iPos = 0;
    let iNeg = 0;
    while (iPos < lPos && iNeg < lNeg) {
        if (getPositive) {
            result.push(positives[iPos]);
            iPos++;
        } else {
            result.push(negatives[iNeg]);
            iNeg++;
        }
